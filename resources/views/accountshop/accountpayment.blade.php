...
<div class="container my-3 mt-10"style="margin-top:100px;">
    ...
    <span class="text-dark fw-bold text-capitalize fs-2">{{ $account->server }} - {{ $account->rank }} <i class="fas fa-angle-right"></i>- {{ $account->number_of_skins }} Skins</span>
    <span class="text-dark fw-semibold text-capitalize fs-5">Val - Account Purchase</span>
    </div>
    </div>
    </section>
    ...
    @php
        $paymentRoute = 'accountpaypalpayment';
    @endphp
@elseif ($paymentMethod === 'stripe')
    @php
        $paymentRoute = 'accountpayment';
    @endphp
@endif

    <div class="d-grid mb-2">
        <a class="btn btn-primary btn-lg" href="{{ route($paymentRoute, ['id' => $account->id]) }}">
    ...
    <script>
        ...
        document.getElementById("stripe").addEventListener("change", function() {
            ...
            document.querySelector(".btn-lg").href = "{{ route('accountpayment', ['id' => $account->id]) }}";
        });
        ...
        document.getElementById("paypal").addEventListener("change", function() {
            ...
            // Update the href of the button
            document.querySelector(".btn-lg").href = "{{ route('accountpaypalpayment', ['id' => $account->id]) }}";
        });
    </script>
@endsection