...
<div class="container my-3 mt-10"style="margin-top:100px;">
    ...
    <span class="text-dark fw-bold text-capitalize fs-2">{{ $winBoostOrder->server }} - {{ $winBoostOrder->current_rank }} <i class="fas fa-angle-right"></i>- {{ $winBoostOrder->wins_number }} Games</span>
    <span class="text-dark fw-semibold text-capitalize fs-5">Val - Win Boost</span>
    </div>
    </div>
    </section>
    ...
    <div class="total-price fw-bold" id="displayed-price">
        {{ $winBoostOrder->total_price }} EUR </div>
    </div>
    </section>
    ...
    @php
        $paymentRoute = 'winboostpaypalpayment';
    @endphp
@elseif ($paymentMethod === 'stripe')
    @php
        $paymentRoute = 'winboostpayment';
    @endphp
@endif

    <div class="d-grid mb-2">
        <a class="btn btn-primary btn-lg" href="{{ route($paymentRoute, ['id' => $winBoostOrder->id]) }}">
    ...
    <script>
        ...
        document.getElementById("stripe").addEventListener("change", function() {
            ...
            document.querySelector(".btn-lg").href = "{{ route('winboostpayment', ['id' => $winBoostOrder->id]) }}";
        });
        ...
        document.getElementById("paypal").addEventListener("change", function() {
            ...
            // Update the href of the button
            document.querySelector(".btn-lg").href = "{{ route('winboostpaypalpayment', ['id' => $winBoostOrder->id]) }}";
        });
    </script>
@endsection