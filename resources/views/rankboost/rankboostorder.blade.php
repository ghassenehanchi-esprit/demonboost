...
<div class="container my-3 mt-10"style="margin-top:100px;">
    ...
    <span class="text-dark fw-bold text-capitalize fs-2">{{ $rankBoostOrder->server }} - {{ $rankBoostOrder->current_rank }} <i class="fas fa-angle-right"></i> {{ $rankBoostOrder->desired_rank }}</span>
    <span class="text-dark fw-semibold text-capitalize fs-5">Val - Rank Boost</span>
    </div>
    </div>
    </section>
    ...
    <div class="total-price fw-bold" id="displayed-price">
        {{ $rankBoostOrder->total_price }} EUR </div>
    </div>
    </section>
    ...
    @php
        $paymentRoute = 'rankboostpaypalpayment';
    @endphp
@elseif ($paymentMethod === 'stripe')
    @php
        $paymentRoute = 'rankboostpayment';
    @endphp
@endif
    <div class="d-grid mb-2">
        <a class="btn btn-primary btn-lg" href="{{ route($paymentRoute, ['id' => $rankBoostOrder->id]) }}">
    ...
    <script>
        ...
        document.getElementById("stripe").addEventListener("change", function() {
            ...
            document.querySelector(".btn-lg").href = "{{ route('rankboostpayment', ['id' => $rankBoostOrder->id]) }}";
        });
        ...
        document.getElementById("paypal").addEventListener("change", function() {
            ...
            // Update the href of the button
            document.querySelector(".btn-lg").href = "{{ route('rankboostpaypalpayment', ['id' => $rankBoostOrder->id]) }}";
        });
    </script>
@endsection